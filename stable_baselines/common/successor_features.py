...
def encode_cnn(scaled_images, is_training=True, reuse=False, **kwargs) -> Tuple[tf.Tensor, tf.Tensor]:
  ...
  """
  # None, 84, 84, 4
  assert scaled_images.shape.as_list() == [BATCH_SIZE, 84, 84, 4], scaled_images.shape
  ...
  with tf.variable_scope('encode', reuse=reuse):
    ...
    assert x.shape.as_list() == [BATCH_SIZE, 40, 40, 64], x.shape
    ...
    x = slim.conv2d(x, 64, 6, 2,
                    ...
                    weights_initializer=weights_initializer,
                    scope='conv2')
    assert x.shape.as_list() == [BATCH_SIZE, 20, 20, 64], x.shape
    ...
    x = slim.conv2d(x, 64, 6, 2,
                    ...
                    weights_initializer=weights_initializer,
                    scope='conv3')
    assert x.shape.as_list() == [BATCH_SIZE, 10, 10, 64], x.shape
...
def decode_cnn(hidden: tf.Tensor, action_ph, is_training=True, reuse=False, *kwargs) -> tf.Tensor:
  batch_norm_fn = functools.partial(slim.batch_norm,
                                    scale=True,
                                    updates_collections=None,
                                    is_training=is_training)
  ...
  assert hidden.shape.as_list() == [BATCH_SIZE, 1024]
  relu = tf.nn.relu
  with tf.variable_scope('decode', reuse=reuse):
    ...
    assert x.shape.as_list() == [BATCH_SIZE, 2048], x.shape

    x = relu(linear(x, 'fc2', n_hidden=1024, init_scale=np.sqrt(2)))  # None, 1024
    assert x.shape.as_list() == [BATCH_SIZE, 1024], x.shape

    x = relu(linear(x, 'fc3', n_hidden=6400, init_scale=np.sqrt(2)))  # 5, 6400
    assert x.shape.as_list() == [BATCH_SIZE, 6400], x.shape
    ...
    assert x.shape.as_list() == [BATCH_SIZE, 84, 84, 4]
  return x
