...
class BaseRLModel(ABC):
  ...
  def set_env(self, env):
    ...
    assert self.action_space == env.action_space, \
      "Error: the environment passed must have at least the same action space as the model was trained on. {} {}".format(
        self.action_space, env.action_space)
...
class TensorboardWriter:
  ...
  def __enter__(self) -> tf.summary.FileWriter:
    if self.tensorboard_log_path is not None:
      ...
      logger.info('TensorBoard Logging to {}'.format(save_path))
