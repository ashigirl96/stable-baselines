...
class SelfImitation(object):
  ...
  def __init__(self, model_ob, model_vf, model_entropy,
    ...
    self.model_vf = model_vf  # type: fc
  ...
  def build_loss_op(self):
    ...
    self.adv = tf.stop_gradient(
      tf.clip_by_value(self.R - tf.squeeze(self.model_vf), 0.0, self.clip))  # policy-SIL
    ...
    delta = tf.clip_by_value(v_estimate - v_target, -self.clip, 0) * mask  # value-SIL
  ...
  def _train(self, sess, lr, update, writer):
  ...
  def train(self, sess, lr, update=None, writer=None):
    if self.n_update == 0:
      return 0, 0, 0, 0
    ...
    if self.n_update < 1:
      ...
      if self.train_count % update_ratio == 0:
        loss, adv, samples, nlogp = self._train(sess, lr, update, writer)
    else:  # n_update > 1
