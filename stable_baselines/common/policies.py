...
def nature_cnn(scaled_images, **kwargs) -> tf.Tensor:
  """
...
class BasePolicy(ABC):
  ...
  def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, n_lstm=256, reuse=False, scale=False,
    ...
    with tf.variable_scope("input", reuse=False):
      ...
      self.action_ph: tf.Tensor = None
      if add_action_ph:
        self.action_ph: tf.Tensor = tf.placeholder(dtype=ac_space.dtype, shape=(None,) + ac_space.shape, name="action_ph")
    self.sess = sess
    self.reuse = reuse
...
class ActorCriticPolicy(BasePolicy):
  ...
  def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, n_lstm=256, reuse=False, scale=False,
    ...
    super(ActorCriticPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, n_lstm=n_lstm,
                                            reuse=reuse, scale=scale, **kwargs)
